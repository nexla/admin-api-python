...
class FlowNodeEnhanced(Base):
    ...
    origin_node = relationship("FlowNodeEnhanced", remote_side=[id], foreign_keys=[origin_node_id])
    parent_node = relationship("FlowNodeEnhanced", remote_side=[id], foreign_keys=[parent_node_id])
    shared_origin_node = relationship("FlowNodeEnhanced", remote_side=[id], foreign_keys=[shared_origin_node_id])
    copied_from = relationship("FlowNodeEnhanced", remote_side=[id], foreign_keys=[copied_from_id])
    child_nodes = relationship("FlowNodeEnhanced", remote_side=[parent_node_id], viewonly=True)
    ...
    def dependencies_satisfied_(self) -> bool:
        ...
        try:
            completed_deps = session.query(FlowNodeEnhanced).filter(
                FlowNodeEnhanced.id.in_(dependency_ids),
                FlowNodeEnhanced.status == NodeStatuses.COMPLETED
            ).count()
    ...
    def __repr__(self) -> str:
        ...
        return f"<FlowNodeEnhanced(id={self.id}, uid='{self.uid}', type='{self.node_type}', status='{self.status}', priority='{self.priority}')>"
