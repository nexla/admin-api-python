module AsyncTasks::Tasks
  class BulkDeleteNotifications < AsyncTasks::Tasks::Base
    def run
      ...
      scope.find_in_batches(batch_size: 500) do |batch|
        ids = batch.map(&:id)
        NotificationsAccessControl.where(notification_id: ids).delete_all
    ...
    def check_preconditions
      if args[:owner_id].present?
        ...
        raise Api::V1::ApiError.new(:not_found, "User ID #{args[:owner_id]} does not exist") unless target_user
